	...
	private function check_for_error($file){//1
		...
		return $this->error = "No file was uploaded.";
		  ...
		  // error: report what PHP says went wrong
			return  $this->error = $this->upload_errors[$file['error']];
				...
				$this->filename = $new_file_name;
				  ...
				  $this->size       = $file['size'];
				   ...
				   if(empty($this->filename) || empty($this->temp_path)) {
						return  $this->error = "The location to save the picture is not available.";
					 ...
					 if (( $this->ext != "JPG") &&( $this->ext != "jpg") && ( $this->ext != "jpeg")&& ( $this->ext != "JPEG") && ( $this->ext != "png")&& ( $this->ext != "PNG") && ( $this->ext != "gif") && ( $this->ext != "GIF") && ( $this->ext != "svg") && ( $this->ext != "SVG")) 
								{
										return  $this->error = "Invalid picture file";
		...
		// Pass in $_FILE(['uploaded_file']) as an argumen
  public function save($file,$new_file_name =NULL,$target) {
	...
	/* if(file_exists( $this->pic_file)) {
		 ...
		 return "error";
			...
			$str_pos = stripos($this->pic_file,$this->ext);
			 $str_len = strlen($this->pic_file) - strlen($this->ext)-1;
			  $temp_name1 = substr($this->pic_file,0,$str_len);
			  $temp_name = str_ireplace('../../../picedit/','',$temp_name1);
			  $db_name = $temp_name . ".".$this->ext;
			...
			return $this->pic_file;
				...
				// File was not moved.
		     $_SESSION['error'] = "Failed to Save File. Pls Try Again";
			 return "error";
